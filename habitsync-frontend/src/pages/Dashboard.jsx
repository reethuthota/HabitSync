...
//     const token = localStorage.getItem('token');
//     try {
//       const [ownHabitsResponse, partnerHabitsResponse] = await Promise.all([
//         axios.get(`${API_BASE_URL}/api/habits`, {
//           headers: { Authorization: `Bearer ${token}` },
//         }),
//         axios.get(`${API_BASE_URL}/api/habits/partner-habits`, {
//           headers: { Authorization: `Bearer ${token}` },
//         }),
//       ]);
  
//       const ownHabits = ownHabitsResponse.data;
//       console.log("IM HERE");
//       console.table(ownHabits);
//       const partnerHabits = partnerHabitsResponse.data;
...
//     const token = localStorage.getItem('token');
//     try {
//       // Call the backend route to log the habit
//       const response = await axios.post(`${API_BASE_URL}/api/habits/${habitId}/log`, {}, {
//         headers: { Authorization: `Bearer ${token}` },
//       });

//       if (response.status === 200) {
//         // Refetch habits after successfully logging
//         fetchHabits(); // Call the function to refresh the habits
//       }
//     } catch (err) {
//       console.error('Error logging habit', err);
//     }
...
//       {/* Your Habits Section with Light Gray Background */}
//       <div className="bg-indigo-50 p-6 rounded-lg shadow-lg mb-6">
//         <div className="flex justify-between items-center mb-4">
//           <h2 className="text-2xl font-semibold text-gray-900">Your Habits</h2>
//           <button
...
//           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
//             {habits.map((habit) => {
//               const lastLogDate = habit.logs?.[habit.logs.length - 1];
//               const today = new Date().toISOString().slice(0, 10);
//               const isLoggedToday = lastLogDate?.slice(0, 10) === today;

//               return (
//                 <div key={habit._id} className="bg-white p-6 rounded-lg shadow-lg hover:shadow-2xl transition duration-300">
...
//       {/* Habits where you are an Accountability Partner Section with Blue Background */}
//       <div className="bg-indigo-50 p-6 rounded-lg shadow-lg">
//         <h2 className="text-2xl font-semibold text-gray-900 mb-4">Habits You Are an Accountability Partner For</h2>
